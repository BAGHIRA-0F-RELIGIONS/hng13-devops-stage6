...
- Deploy step: SCP -> extract -> copy to `/opt/todo-app` on the server, installs docker/docker-compose, then `docker-compose up -d --build`.
...
**Rollback / Restore**
- The deployment job creates backups of `/opt/todo-app` under `/opt/backups/todo-app-<timestamp>` before replacing — to rollback, stop containers and restore the desired backup, then `docker compose up -d`.
